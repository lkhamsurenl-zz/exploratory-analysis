...
mortality_and_birth_rate <- merge(
  ...
  mutate(
    infant_death_rate = total_infant_deaths / num_of_births
  )
```
...
mortality_and_birth_rate %>% 
  filter(Country %in% c('Mongolia', 'United States of America', 'Mexico', 'Sweden', 'India', 'China')) %>% 
  ggplot(aes(x = Year, y = infant_death_rate, color = Country)) + 
    geom_line()
```
...
merge(
  ...
  ggplot(aes(x = healthcare_expenditure_perc_gdp, y = infant_death_rate, label = Country)) + 
    geom_point() + 
    geom_text()
```
